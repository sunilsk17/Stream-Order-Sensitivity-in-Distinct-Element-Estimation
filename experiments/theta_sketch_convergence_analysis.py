#!/usr/bin/env python3
"""
Theta Sketch Cardinality Analysis - Real Public Datasets

Analyzes order sensitivity of Theta Sketch algorithm on the same
...
# Import Theta Sketch implementation
import sys
sys.path.insert(0, str(Path(__file__).parent.parent))
from sketches.theta_sketch import ThetaSketch
...
def analyze_convergence(items, dataset_name, ordering_name, k=4096):
    """
    Run convergence test with Theta Sketch.
...
def main():
    """Run Theta Sketch analysis on all datasets."""
    print("=" * 80)
    print("THETA SKETCH CARDINALITY ANALYSIS - REAL PUBLIC DATASETS")
    print("=" * 80)
    print()
    print("Algorithm: Theta Sketch")
    print("Sketch Parameter: k = 4096 (comparable to HLL p=10)")
    print()
    ...
    for dataset_name, dataset_info in loaded_datasets.items():
        ...
        for ordering_name in ['grouped', 'random', 'chrono']:
            ...
            result = analyze_convergence(items, dataset_name, ordering_name, k=4096)
    ...
    output_path = Path('results/theta_sketch_convergence_analysis_results.json')
    with open(output_path, 'w') as f:
        json.dump(output, f, indent=2)
    
    print("\n" + "=" * 80)
    print("SENSITIVITY SPECTRUM SUMMARY - THETA SKETCH")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("KEY FINDINGS - THETA SKETCH")
    print("=" * 80)
    ...
    print("✓ Sketch parameter k=4096 (comparable to HLL p=10)")
    print("✓ Results show Theta Sketch order sensitivity patterns")
